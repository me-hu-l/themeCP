...
import { useContestHistory } from '../../context/ProfileContext/ContestHistoryContext'
...
const ImportExport = ({id}) => {
    ...
    useEffect(() => {
      const fetchContests = async()=>{
        try {
          const response = await fetch(`${backend_url}/api/contests/contestHistory/${id}`, {
            method: 'GET',
            credentials: 'include',
          });
          if (!response.ok) {
    ...
    const columnsToIgnore = [ "id","user_id", "contest_no", "created_at"];
