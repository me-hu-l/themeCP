...
const Contest = () => {
  ...
  // useEffect(() => {
  //   console.log('Level changed:', problemLevel.current.value);
  //   console.log('Problem Array:', problemArray);
  //   console.log('Duration:', contest_duration);
  // }, [problemLevel.current.value, problemArray, contest_duration]);
  ...
  const check = async () => {
    ...
    const get_username = await fetch(`${backend_url}/api/users/me`, {
      credentials: 'include'
    }) .then((res) => res.json()); 
    console.log('checking user handle:', get_username.codeforces_handle);
    if (get_username.codeforces_handle === null) {
      alert('ADD YOUR CODEFORCES HANDLE');
      router.push(`/profile/${get_username.id}`);
    } else {
    ...
    const api_call = async () => {
      const backend_url = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
      const contest_history = await fetch(`${backend_url}/api/contests/me`, {
        credentials: 'include'
      }) .then((res) => res.json());
  ...
  const reroll = (index) => {
    ...
    <center>
      <div className='main-container'>
        ...
        {!themecp_created ? <p style={{textAlign: 'left', fontSize:'16px', marginBottom:'30px', color:'red'}}>If suggested level is too easy/hard, you can choose an appropriate level from <Link href='/level_sheet'><span style={{fontSize:'17px'}}>level sheet</span></Link></p> : ''}
